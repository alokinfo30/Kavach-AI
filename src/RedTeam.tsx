  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold leading-7 text-gray-900 dark:text-white sm:truncate sm:text-3xl sm:tracking-tight">Red Team Lab</h2>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Manage and execute adversarial attacks against your AI models.</p>
        </div>
        <div className="flex gap-2">
          {selectedTests.length === 2 && (
            <button
              onClick={() => navigate(`/red-team/compare?test1=${selectedTests[0]}&test2=${selectedTests[1]}`)}
              className="inline-flex items-center gap-x-2 rounded-md bg-white dark:bg-gray-800 px-3.5 py-2.5 text-sm font-semibold text-gray-900 dark:text-white shadow-sm ring-1 ring-inset ring-gray-300 dark:ring-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700"
            >
      ...
      {showForm && (
        <div className="rounded-lg bg-white dark:bg-gray-800 p-6 shadow-sm border border-gray-200 dark:border-gray-700">
          <form onSubmit={handleCreate} className="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-4 items-end">
             <div>
               <label className="block text-sm font-medium text-gray-900 dark:text-white">Test Name</label>
               <input type="text" required value={newTest.test_name} onChange={e => setNewTest({...newTest, test_name: e.target.value})} className="mt-1 block w-full rounded-md border-0 py-1.5 px-3 text-gray-900 dark:text-white dark:bg-gray-700 ring-1 ring-inset ring-gray-300 dark:ring-gray-600 placeholder:text-gray-400 sm:text-sm sm:leading-6" placeholder="e.g. Alpha Model Injection" />
             </div>
             <div>
               <label className="block text-sm font-medium text-gray-900 dark:text-white">Type</label>
               <select value={newTest.test_type} onChange={e => setNewTest({...newTest, test_type: e.target.value})} className="mt-1 block w-full rounded-md border-0 py-1.5 px-3 text-gray-900 dark:text-white dark:bg-gray-700 ring-1 ring-inset ring-gray-300 dark:ring-gray-600 sm:text-sm sm:leading-6">
             ...
             <div>
               <label className="block text-sm font-medium text-gray-900 dark:text-white">Target System</label>
               <input type="text" required value={newTest.target_system} onChange={e => setNewTest({...newTest, target_system: e.target.value})} className="mt-1 block w-full rounded-md border-0 py-1.5 px-3 text-gray-900 dark:text-white dark:bg-gray-700 ring-1 ring-inset ring-gray-300 dark:ring-gray-600 placeholder:text-gray-400 sm:text-sm sm:leading-6" placeholder="e.g. GPT-4 Proxy" />
             </div>
      ...
      <div className="overflow-hidden bg-white dark:bg-gray-800 shadow sm:rounded-md border border-gray-200 dark:border-gray-700">
        <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex flex-wrap justify-between items-center bg-gray-50 dark:bg-gray-700/50 gap-4">
          <div className="flex items-center gap-4">
            <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-200 uppercase tracking-wider">Test Inventory</h3>
            {statusFilter !== 'all' && (
              <button 
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <label className="text-xs text-gray-500 dark:text-gray-400 font-medium">Filter:</label>
              <select 
                value={statusFilter} 
                onChange={(e) => setStatusFilter(e.target.value)}
                className="text-xs border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded py-1"
              >
        ...
        <ul role="list" className="divide-y divide-gray-200 dark:divide-gray-700">
          {loading ? (
            <li className="p-8 text-center text-gray-500">Loading tests...</li>
          ) : filteredTests.length === 0 ? (
            <li className="p-8 text-center text-gray-500 dark:text-gray-400">{tests.length === 0 ? 'No tests found. Click "New Test" to create one.' : 'No tests matching current filter.'}</li>
          ) : (
            filteredTests.map((test) => (
              <li key={test.id} className="px-4 py-4 sm:px-6 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                <div className="flex items-center justify-between">
                  ...
                  <div className="min-w-0 flex-1">
                     <div className="flex items-center gap-x-3">
                       <p className="truncate text-sm font-medium text-indigo-600 dark:text-indigo-400">{test.test_name}</p>
                       <span className={cn("inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset", 
                          test.status === 'completed' ? "bg-green-50 text-green-700 ring-green-600/20 dark:bg-green-950/40 dark:text-green-400" :
                          test.status === 'running' ? "bg-blue-50 text-blue-700 ring-blue-600/20 dark:bg-blue-950/40 dark:text-blue-400" :
                          "bg-yellow-50 text-yellow-800 ring-yellow-600/20 dark:bg-yellow-950/40 dark:text-yellow-400"
                       )}>
                         {test.status}
                       </span>
                     </div>
                     <div className="mt-2 flex flex-wrap items-center text-sm text-gray-500 dark:text-gray-400 gap-x-4">
                  ...
                  <div className="flex items-center gap-x-2">
                    {test.status === 'pending' || test.status === 'completed' || test.status === 'failed' ? (
                      <button 
                        ...
                        className="rounded bg-indigo-50 dark:bg-indigo-900/40 px-2.5 py-1.5 text-xs font-semibold text-indigo-600 dark:text-indigo-400 shadow-sm hover:bg-indigo-100 disabled:opacity-50 transition-colors"
                      >
                    ...
                    {test.results && (
                      <button 
                        ...
                        className="rounded bg-gray-50 dark:bg-gray-700 px-2.5 py-1.5 text-xs font-semibold text-gray-600 dark:text-gray-300 shadow-sm hover:bg-gray-100 flex items-center gap-1 transition-colors"
                      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 p-4 backdrop-blur-sm">
            <div className="w-full max-w-2xl bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 max-h-[90vh] overflow-y-auto">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white">{selectedTestDetails.test_name} Results</h3>
                ...
                <div className="mt-6 flex justify-end gap-2">
                     <button onClick={() => setSelectedTestDetails(null)} className="rounded-md bg-gray-100 dark:bg-gray-700 px-4 py-2 text-sm font-semibold text-gray-900 dark:text-white hover:bg-gray-200 dark:hover:bg-gray-600">Close</button>
                </div>
            </div>
        </div>
      )}
